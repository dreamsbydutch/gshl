...
import { Seasons, LockerRoomPlayerStatPropsType, PlayerContractType, PlayerSplitsType, QueryKeyType, TeamDraftPickType, TeamInfoType, PlayerCurrentRosterType } from '../utils/endpointTypes'
...
export default function LockerRoom() {
  ...
  const playerSeasonData: PlayerSplitsType[] | null = usePlayerSplits()
  ...
  const currentRosterData: PlayerCurrentRosterType[] = currentRosterResponse.data
  ...
  const gshlTeamsCapSpace: salaryCapTotalsType[] | undefined = gshlTeams.map(obj => {
    const temp = { ...obj, 'capSpace': 22500000 - sumObjectByKey(contractData.filter(a => a.CurrentTeam === obj[seasons[0].Season] && +a.YearsRemaining > 0), 'CapHit') }
    return temp
  })
                ...
                {gshlTeamsCapSpace?.filter(obj => obj[seasons[0].Season]).sort((a, b) => b.capSpace - a.capSpace).map(team => {
                  ...
                  return (
                    <tr key={team.id}>
                      ...
                      <td className='px-2 py-1 text-xs font-normal text-center text-gray-800 whitespace-nowrap'>{formatter.format(22500000 - sumObjectByKey(contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 0), 'CapHit')).replace("US", "")} ({contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 0 && obj.ExpiryType !== 'Buyout').length})</td>
                      <td className='px-2 py-1 text-xs font-normal text-center text-gray-800 whitespace-nowrap'>{formatter.format(25000000 - sumObjectByKey(contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 1), 'CapHit')).replace("US", "")} ({contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 1 && obj.ExpiryType !== 'Buyout').length})</td>
                      <td className='px-2 py-1 text-xs font-normal text-center text-gray-800 whitespace-nowrap'>{formatter.format(25000000 - sumObjectByKey(contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 2), 'CapHit')).replace("US", "")} ({contractData?.filter(obj => obj.CurrentTeam === team[seasons[0].Season] && +obj.YearsRemaining > 2 && obj.ExpiryType !== 'Buyout').length})</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <table className='mx-auto overflow-x-auto'>
          ...
          <tbody>
            ...
            <tr key='CapSpace'>
              ...
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">{formatter.format(25000000 - currentTeamContracts?.filter(obj => obj.YearsRemaining > 1).reduce((acc, obj) => acc + +obj.CapHit, 0)).replace("US", "")}</td>
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">{formatter.format(25000000 - currentTeamContracts?.filter(obj => obj.YearsRemaining > 2).reduce((acc, obj) => acc + +obj.CapHit, 0)).replace("US", "")}</td>
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">{formatter.format(25000000 - currentTeamContracts?.filter(obj => obj.YearsRemaining > 3).reduce((acc, obj) => acc + +obj.CapHit, 0)).replace("US", "")}</td>
            </tr>
          </tbody>
        </table>
      </div>
