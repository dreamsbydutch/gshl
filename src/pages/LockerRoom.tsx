...
export default function LockerRoom() {
  ...
  return (
    <div className="my-4 font-varela text-center">
      <TeamsToggle
        {...{ setter: setTeamID, activeKey: teamID, toolbarKeys: gshlTeams.currentTeams }}
      />
      {teamID && (
        <>
          ...
          <div className="font-bold text-2xl flex flex-col items-center justify-center mb-4">
            <img
              className="max-w-xs w-2/6 h-2/6 mx-auto"
      ...
      {!teamID && (
        <>
          <div className="mt-8 text-center mx-auto text-xl font-bold">
            GSHL Salary Caps
          </div>
          <div className="table-auto overflow-scroll no-scrollbar">
            <table className="mx-auto overflow-x-auto border border-black">
              <thead>
                <tr key="header">
                  <th className="sticky left-0 p-1 text-2xs font-normal text-center bg-gray-800 text-gray-200">
                    Team
                  </th>
                  <th className="p-1 text-2xs font-normal text-center bg-gray-800 text-gray-200">
                    2023-24
                  </th>
                  <th className="p-1 text-2xs font-normal text-center bg-gray-800 text-gray-200">
                    2024-25
              ...
              <tbody>
                {gshlTeamsCapSpace
                  ?.filter((obj) => obj[seasons[0].Season])
                      ...
                      <tr key={team.id}>
                        <td className="sticky left-0 flex justify-start px-2 py-1 text-sm font-normal text-gray-800 bg-gray-50 whitespace-nowrap">
                          <div className="place-self-center overflow-hidden">
                            <img
                              src={team.LogoURL}
                              alt="NHL Team Logo"
                              className="h-6 w-6 mx-2 inline-block"
                            />
                            {team.TeamName}
                          </div>
                        </td>
                        <td className="px-2 py-1 text-xs font-normal text-center text-gray-800 whitespace-nowrap">
                              ...
                              22500000 -
                                sumObjectByKey(
                                  contractData?.filter(
                                    (obj) =>
                                      obj.CurrentTeam ===
                                        team[seasons[0].Season] &&
                                      +obj.YearsRemaining > 0 &&
                                      obj.ExpiryType !== "Buyout"
                                  ),
                                  "CapHit"
                                )
                            )
                          ...
                          (
                          {
                            contractData?.filter(
                              (obj) =>
                                obj.CurrentTeam === team[seasons[0].Season] &&
                                +obj.YearsRemaining > 0 &&
                                obj.ExpiryType !== "Buyout"
                            ).length
                          }
...
function TeamPlayerContracts(props: LockerRoomPlayerStatPropsType) {
  const currentTeamContracts = props.teamContracts?.filter(
    (obj) =>
      +obj.YearsRemaining > 0 ||
      (+obj.YearsRemaining === 0 && obj.ExpiryType !== "Buyout")
  );
  let formatter = new Intl.NumberFormat(navigator.language, {
    style: "currency",
    currency: "USD",
  ...
  return (
    <>
      <div className="text-center mx-auto text-xl font-bold">
      ...
      <div className="table-auto overflow-scroll no-scrollbar">
        <table className="mx-auto overflow-x-auto">
          ...
          <tbody>
              ...
              .map((obj, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {+obj.YearsRemaining > 0 ? (
                      <td className="py-1 px-2 text-center text-xs border-t border-b border-gray-300">
                        {formatter.format(obj.CapHit).replace("US", "")}
                      </td>
                    ) : +obj.YearsRemaining === 0 ? (
                      <td
                      ...
                      >
                        {obj.ExpiryType === "Buyout" ? "" : obj.ExpiryType}
                      </td>
                    ) : (
            ...
            <tr key="CapSpace">
              <td className="sticky left-0 font-bold py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">
                Cap Space
              </td>
              ...
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">
                {formatter
                  .format(
                    22500000 -
                      currentTeamContracts
                        ?.filter((obj) => obj.YearsRemaining > 0)
              ...
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">
                {formatter
                  .format(
                    25000000 -
                      currentTeamContracts
                        ?.filter((obj) => obj.YearsRemaining > 1)
                        .reduce((acc, obj) => acc + +obj.CapHit, 0)
                  )
                  .replace("US", "")}
              </td>
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">
                {formatter
                  .format(
                    25000000 -
                      currentTeamContracts
                        ?.filter((obj) => obj.YearsRemaining > 2)
                        .reduce((acc, obj) => acc + +obj.CapHit, 0)
                  )
                  .replace("US", "")}
              </td>
              <td className="py-1 px-2 text-center text-xs border-t border-gray-800 bg-gray-200">
                {formatter
                  .format(
                    25000000 -
                      currentTeamContracts
                        ?.filter((obj) => obj.YearsRemaining > 3)
                        .reduce((acc, obj) => acc + +obj.CapHit, 0)
                  )
                  .replace("US", "")}
              </td>
...
function TeamDraftPicks(props: LockerRoomPlayerStatPropsType) {
  const gshlTeams = useTeams();
  const draftPickQueryKey: QueryKeyType = [
    seasons[0].Season,
    "TeamData",
    "DraftPicks",
  ];
  ...
  const teamDraftPicks = draftPickData?.filter(
    (obj) =>
      props.teamInfo && obj.gshlTeam === props.teamInfo[seasons[0].Season]
  );
      ...
      {teamDraftPicks
        ...
        .map((obj, i) => {
          ...
          if (obj.OriginalTeam !== obj.gshlTeam) {
            originalTeam = gshlTeams?.filter(
              (x) => x[seasons[0].Season] === obj.OriginalTeam
            )[0];
          }
            ...
            return (
              <div key={i + 1} className="text-gray-800">
                <div className="mx-auto w-5/6 py-1 px-2 text-center text-xs border-t border-gray-300">
                  ...
                  {obj.Pick + numberSuffix(+obj.Pick)} Overall
          ...
          return (
            <div key={i + 1} className="text-gray-400">
              <div className="mx-auto w-5/6 py-1 px-2 text-center text-xs border-t border-gray-300">
                {currentTeamContracts[teamDraftPicks.length - i - 1].PlayerName}
                , {currentTeamContracts[teamDraftPicks.length - i - 1].Pos} (
                {obj.Rd + numberSuffix(+obj.Rd)} Round)
              </div>
            </div>
          );
...
function TeamPlayerStats(props: LockerRoomPlayerStatPropsType) {
  const teamPlayers = props.playerSeasonData?.filter(
    (obj) =>
      props.teamInfo &&
      obj.gshlTeam === props.teamInfo[seasons[0].Season] &&
      obj.WeekType === "RS"
  );
  const currentRoster = props.currentRosterData?.filter(
    (obj) =>
      props.teamInfo && obj.gshlTeam === props.teamInfo[seasons[0].Season]
  );
  console.log(props);
        ...
        <table className="mx-auto overflow-x-auto">
              ...
              .map((obj) => {
                return (
                  <tr key={obj.id}>
                    ...
                    <td className="py-1 px-1.5 text-center text-xs border-t border-b border-gray-300">
                      {obj.G}
                    </td>
                    <td className="py-1 px-1.5 text-center text-xs border-t border-b border-gray-300">
                      {obj.A}
          ...
          <thead>
            <tr>
              ...
              <th
                colSpan={2}
                className="p-1 text-2xs font-normal text-center bg-gray-800 text-gray-200"
              >
                W
              </th>
              <th
              ...
              .map((obj) => {
                return (
                  <tr key={obj.id}>
                    ...
                    <td
                      colSpan={2}
                      className="py-1 px-1.5 text-center text-xs border-t border-b border-gray-300"
                    >
                      {obj.W}
                    </td>
                    <td
